 ...
 * 🔐 Clerk Authentication + API Security (rate limiting, CORS, XSS/SQL detection)
 *
...
export default clerkMiddleware(async (auth, request) => {
  ...
  if (isAdminRoute(request)) {
    ...
    if (userEmail !== 'rey7214935@gmail.com') {
      return NextResponse.json(
        { success: false, error: 'Acceso denegado. Solo el administrador.' },
        { status: 200 }
      )
    }
  }

  // ── API routes: apply security ──────────────────────────────────────
  if (pathname.startsWith('/api/')) {
    ...
    if (!rateLimitResult.allowed) {
      ...
      const response = NextResponse.json(
        { success: false, error: 'Demasiadas solicitudes.', retryAfter },
        {
          status: 200,
          headers: {
    ...
    if (maliciousCheck.isSQL || maliciousCheck.isXSS) {
      console.warn('[Security] Blocked:', {
        ip: clientIP, method: request.method, path: pathname,
        type: maliciousCheck.isSQL ? 'SQL_INJECTION' : 'XSS',
      })
      const response = NextResponse.json(
        { success: false, error: 'Solicitud bloqueada por seguridad.' },
        { status: 200 }
      )
      const origin = request.headers.get('origin')
      const corsHeaders = getCorsHeaders(origin)
