...
/**
 * Modular seed function that can be called from:
 * 1. The CLI: `bun run prisma/seed.ts`
 * 2. The setup API: POST /api/setup
 *
 * This is separated from prisma/seed.ts so it can be imported
 * by the API route without the self-executing main() pattern.
 */
export async function seedDatabase(): Promise<{
  ...
  await db.quantityDiscount.deleteMany()
  await db.businessProfile.deleteMany()
  await db.advertisement.deleteMany()
  await db.commissionLog.deleteMany()
  ...
  const userCount = 3 + suppliers.length + 1 // 3 demo sellers + 19 suppliers + 1 buyer
  const productCount = products.length + suppliers.reduce((acc, s) => acc + s.products.length, 0)

  console.log(`✅ Seeded ${productCount} products, ${3 + suppliers.length} sellers (3 demo + ${suppliers.length} verified), 1 buyer`)
