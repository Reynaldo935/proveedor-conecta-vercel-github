  ...
  try {
    ...
    if (format === 'csv') {
      ...
      const rows = transactions.map(t =>
        `"${new Date(t.createdAt).toLocaleDateString('es-NI')}","${t.product.title}","${t.product.category}",${t.amount},"${t.paymentMethod}","${t.status}","${t.seller.businessProfile?.businessName || t.seller.name}"`
      ).join('\n')
...
${transactions.map(t => `<tr><td>${new Date(t.createdAt).toLocaleDateString('es-NI')}</td><td>${t.product.title}</td><td>${t.product.category}</td><td>${t.amount}</td><td>${t.paymentMethod}</td><td>${t.status}</td><td>${t.seller.businessProfile?.businessName || t.seller.name}</td></tr>`).join('')}
</table></body></html>`
...
${transactions.map(t => `<tr><td>${new Date(t.createdAt).toLocaleDateString('es-NI')}</td><td>${t.product.title}</td><td>${t.product.category}</td><td class="price">${formatPrice(t.amount)}</td><td>${t.paymentMethod}</td><td class="status-${t.status.toLowerCase()}">${t.status === 'COMPLETED' ? 'Completado' : t.status === 'PENDING' ? 'Pendiente' : t.status}</td><td>${t.seller.businessProfile?.businessName || t.seller.name}</td></tr>`).join('')}
</table>
