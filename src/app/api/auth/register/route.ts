...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || name.trim().length === 0) {
      return NextResponse.json({ success: false, error: 'El nombre es requerido' }, { status: 200 })
    }

    // Validate email
    if (!email) {
      return NextResponse.json({ success: false, error: 'El correo electrónico es requerido' }, { status: 200 })
    }
    const emailValidation = validateEmail(email)
    if (!emailValidation.valid) {
      return NextResponse.json({ success: false, error: emailValidation.message }, { status: 200 })
    }

    // Validate password
    if (!password) {
      return NextResponse.json({ success: false, error: 'La contraseña es requerida' }, { status: 200 })
    }
    ...
    if (existing) {
      return NextResponse.json({ success: false, error: 'El correo ya está registrado' }, { status: 200 })
    }
    ...
    return NextResponse.json({ success: false, error: 'Error al registrar usuario' }, { status: 200 })
  }
}
